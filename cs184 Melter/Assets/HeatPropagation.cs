...
//Heat Propagation demo script.

public class HeatPropagation : MonoBehaviour
{

    // Lattice structure definitions
    public float width = 117f, height = 3f, depth = 3f;
    public int num_w = 40, num_h = 2, num_d = 2;
    ...
    void Start () {
      ...
      // Set initial temperatures and update colors.
      for (int j = 0; j < num_h; j++) {
        for (int k = 0; k < num_d; k++) {
          spheres[0,j,k].GetComponent<SphereTemp>().temp = 100f;
          spheres[num_w-1,j,k].GetComponent<SphereTemp>().temp = 0f;
        }
      }
      updateColors();
    }
    ...
    void Update()
    {
      ...
      if (start && timer > timeStep) {
        ...
        updateTemps();
        //updateSprings(); //For this demo, we don't update the springs to simulate the melting behavior.
        vibrateSpheres();
        updateColors();
      }
    }
